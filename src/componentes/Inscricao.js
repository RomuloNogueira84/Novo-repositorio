{
  /*Importando algumas importes necessários*/
}
...
function Inscricao() {
  ...
  {
    /*Construindo Variaveis de Estado com useState*/
  }
  ...
  {
    /*Função Enviar Responsável por simula o envio do formulário*/
  }
  const enviar = (e) => {
    e.preventDefault();
    ...
    // Validação do endereço não pode ser vazio
    const enderecoValido = endereco.length !== 0;
    // Validação do Estado não pode ser vazio;
      const EstadoValido = estado.length !== 0;
    // Validação da  cidade
    const cidadeValido = cidade.length !== 0;
        // Criação da variavel mensagem
    let mensagem = "";

    {
      /*Verifica cada campo do Formulario*/
    }
          ...
          if (!enderecoValido) {
            ...
            if (!cidadeValido) {
              ...
              if (!EstadoValido) {
                ...
                {
                  /*Se Todas as Condições forem satisfeitas então simula o envio*/
                }
                setNome("");
                setEmail("");
                setCidade("");
                ...
                setEstado("");
                {
                  /*Status muda para falso oque faz a troca do componente formulario por um alerta*/
                }
                setStatus(false);
                {
                  /*A função setTimeout espera 10 segundos para fazer o formulário aparece */
                }
                setTimeout(() => {
                  setStatus(true);
                }, 10000);
    ...
    {
      /*Se apresentar algum erro então ira grama um alerta */
    }
